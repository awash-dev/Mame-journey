...
import { FaSearch, FaBars, FaTimes, FaPen } from "react-icons/fa";
...
const Navbar: React.FC = () => {
  ...
  const NavLink = ({ href, label }: { href: string; label: string }) => (
    <Link href={href} onClick={() => setMobileMenuOpen(false)} className="text-lg hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
      {label}
    </Link>
  );
  ...
  const MobileDarkModeSwitch: React.FC = () => (
    ...
    >
      <AnimatePresence mode="wait">
        {isDark ? (
          <motion.span key="moon-mobile" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }} transition={{ duration: 0.2 }}>
            <BsMoonStarsFill className="w-5 h-5 text-gray-300" />
          </motion.span>
        ) : (
          <motion.span key="sun-mobile" initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.5 }} transition={{ duration: 0.2 }}>
  ...
  const TextareaModal = () => (
    <motion.div
      ...
      className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white dark:bg-gray-800 shadow-2xl rounded-xl p-6 w-full max-w-md h-[300px] flex flex-col"
    >
      <h2 className="text-lg font-semibold mb-2 text-black dark:text-white">Write Message</h2>
      <textarea